...
import { fetchPinnedComment, fetchRegularComments, addComment, uploadImage } from '../utils/dataService';
...
const Komentar = () => {
    ...
    useEffect(() => {
        const loadPinnedComment = async () => {
            try {
                const data = await fetchPinnedComment();
                if (data) {
    ...
    useEffect(() => {
        const loadComments = async () => {
            try {
                const data = await fetchRegularComments();
                setComments(data || []);
            } catch (error) {
    ...
    const handleCommentSubmit = useCallback(async ({ newComment, userName, imageFile }) => {
        ...
        try {
            ...
            // Show success message for local mode
            setError('Comment functionality is disabled in local mode. Your comment would have been: "' + newComment + '"');
        } catch (error) {
            setError('Comment functionality is disabled in local mode.');
